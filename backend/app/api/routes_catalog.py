...
from app.api.deps import get_family_currency
...
async def list_catalog(
    family: Family = Depends(get_family_currency), db: AsyncSession = Depends(get_db)
) -> list[AssetOut]:
...
async def get_asset(
    symbol: str, family: Family = Depends(get_family_currency), db: AsyncSession = Depends(get_db)
) -> AssetDetailOut:
