...
/** The home-screen skeleton, shared by `home/loading.tsx` (first hard
 * load of a `/home/*` route) and `FamilyProvider`'s Suspense fallback
 * (while the one-time `/home` seed request is in flight). Once the client
 * store is populated, navigation between cached screens is instant and
 * neither of these is seen again for the session. */
...
/** Shared by `buy/[symbol]/loading.tsx` and the buy screen orchestrator
 * while the asset detail (price, chart) is still loading. */
